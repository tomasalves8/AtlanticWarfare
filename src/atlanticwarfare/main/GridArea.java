...
public class GridArea extends JPanel {
  ...
  public GridArea(String title, Game board) {
    ...
    for (int y = 0; y < 10; y++) for (int x = 0; x < 10; x++) gridRects[x][y] =
      new Rectangle(x * 25, y * 25, 25, 25);
  ...
  public Dimension getPreferredSize() {
    return new Dimension(30 * 10, 30 * 10);
  }
      ...
      if (ver) {
        for (int j = -1 + location.y; j <= shipSize + location.y; j++) {
          for (int j2 = -1 + location.x; j2 <= 1 + location.x; j2++) {
            if (
              j >= location.y && j < shipSize + location.y && j2 == location.x
            ) {
              ...
              if (
                getArea(new Point(j2, j), areatocheck) != 0 &&
                getArea(new Point(j2, j), areatocheck) != 1
              ) {
                return false;
              }
            }
        ...
        for (int j = -1 + location.y; j <= 1 + location.y; j++) {
          for (int j2 = -1 + location.x; j2 <= shipSize + location.x; j2++) {
            if (
              j2 >= location.x && j2 < shipSize + location.x && j == location.y
            ) {
              if (getArea(new Point(j2, j), areatocheck) != 0) {
                return false;
              }
    ...
    if (shipsVisible) {
      for (int y = 0; y < 10; y++) for (int x = 0; x < 10; x++) {
        ...
        if (area[y][x] != 0) {
          ...
          if (ship != 0) {
            if (
              (!isInGrid(y - 1, x) || getGridShip(x, y - 1) != ship) &&
              (!isInGrid(y, x + 1) || getGridShip(x + 1, y) != ship) &&
              (!isInGrid(y, x - 1) || getGridShip(x - 1, y) != ship)
            ) {
  ...
  private class MouseHandler extends MouseAdapter {
    ...
    public void mousePressed(MouseEvent e) {
      if (e.getButton() == MouseEvent.BUTTON1) {
        ...
        if (
          getTitle().equals("Home Field") &&
          player.getSelectedShip() != GameType.IDLE
        ) {
          ...
          getTitle().equals("Opponent's Field") &&
          board.gameStarted &&
          getOpponent().canFire() &&
          firedUpon(selected) != 0
        ) {
          fire();
        }
      }
    ...
    for (int i = 0; i < 10; i++) {
      for (int j = 0; j < 10; j++) {
        for (int shipAlive : getOpponent().shipsAlive) {
          for (Point point : placedShipCells(
            new Point(i, j),
            shipAlive,
            true,
            placed
  ...
  public ArrayList<Point> placedShipCells(
    Point initialpoint,
    int ship,
    boolean vertical,
    int[][] areatocheck
    ...
    if (validPlacement(ship, initialpoint, vertical, areatocheck)) {
      for (int i = 0; i < shipSize; i++) {
        ...
        if (makesSensetoFire(shot)) points.add(shot);
      }
    }
  ...
  public Point getBestShot() {
    ...
    for (int i = 0; i < 10; i++) {
      for (int j = 0; j < 10; j++) {
        if (
          probgrid[i][j] > highestprob &&
          makesSensetoFire(new Point(j, i)) &&
          probgrid[i][j] > 0
        ) {
  ...
  public void fire() {
    ...
    for (
      Iterator<Point> iterator = queuedShots.iterator();
      iterator.hasNext();
    ) {
          ...
          if (shotstatus == 2) {
            if (newi == 0 || newi == 1) {
              ...
              if (newi == 0) {
                for (
                  int j = 1;
                  j <= GameType.getShipSize(maxOpponentShipSize()) - 1;
                  j++
                ) {
